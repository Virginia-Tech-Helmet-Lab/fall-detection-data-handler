...
const BoundingBoxTool = ({ 
  ...
  useEffect(() => {
    if (videoId) {
      fetch(`${basePath}/api/bbox-annotations/${videoId}`)
        .then(response => {
          if (!response.ok) {
  ...
  const saveBoundingBox = async (box) => {
    try {
      ...
      const response = await fetch(`${basePath}/api/bbox-annotations`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          height: box.height,
          part_label: selectedLabel,
          annotator_name: localStorage.getItem('annotator_name') || null,
        }),
      });
  ...
  const deleteBox = async (boxId) => {
    try {
      ...
      console.log(`Attempting to delete box with ID ${boxId} using URL: /api/remove-bbox/${boxId}`);
      
      // Use a different URL path
      const response = await fetch(`${basePath}/api/remove-bbox/${boxId}`);
