...
const NormalizationPanel = () => {
    ...
    useEffect(() => {
        if (selectedVideo) {
            ...
            setPreviewUrl(`${basePath}/api/static/${selectedVideo.filename}`);
    ...
    const generatePreview = async () => {
        ...
        try {
            const response = await apiClient.post('/api/preview-normalize', {
                video_id: selectedVideo.video_id,
                settings: settings
            });
            
            // Use the API endpoint for serving preview videos
            setNormalizedPreviewUrl(`${basePath}/api/preview/${response.data.preview_filename.split('/').pop()}?t=${Date.now()}`);
    ...
    const applyNormalization = async () => {
        ...
        try {
            if (applyToAll) {
                const response = await apiClient.post('/api/normalize-all', {
                    settings: settings
                });
                setMessage(`Normalized ${response.data.processed_count} videos successfully!`);
            } else {
                const response = await apiClient.post('/api/normalize', {
                    video_id: selectedVideo.video_id,
                    settings: settings
                });
